...
public class WarehouseServiceImpl implements WarehouseService {
    ...
    private List<WarehouseCountingVO> res_ = new ArrayList<>();
...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public int getWarehouseInputProductQuantityByTime(String beginDateStr,String endDateStr){
        ...
        try{
            ...
            if (warehouseInputSheetDao.getWarehouseInputProductQuantityByTime(beginTime,endTime) == null){
                return 0;
            }
            else{
    ...
    public int getWarehouseOutProductQuantityByTime(String beginDateStr,String endDateStr){
        ...
        try{
            ...
            if (warehouseOutputSheetDao.getWarehouseOutputProductQuantityByTime(beginTime,endTime) == null){
                return 0;
            }
            else{
    ...
    public Object exportExcelPaper() {
        List<WarehouseCountingVO> list = res_;
        // 标题
        String[] title = { "编号", "名称", "型号", "库存数量", "进价", "零售价", "最近进价", "最近零售价" };
        // 创建一个工作簿
        ...
        for (int i = 0;i < list.size();i++){
            ...
            for (int j = 0; j < title.length; j++) {
                ...
                switch (j){
                    case 0:
                        value = list.get(i).get_product().get_id();
                        break;
                    case 1:
                        value = list.get(i).get_product().get_name();
                        break;
                    case 2:
                        value = list.get(i).get_product().get_type();
                        break;
                    case 3:
                        value = list.get(i).get_product().get_quantity().toString();
                        break;
                    case 4:
                        value = list.get(i).get_product().get_purchasePrice().toString();
                        break;
                    case 5:
                        value = list.get(i).get_product().get_retailPrice().toString();
                        break;
                    case 6:
                        value = list.get(i).get_product().get_recentPp().toString();
                        break;
                    case 7:
                        value = list.get(i).get_product().get_recentRp().toString();
                        break;
                }
