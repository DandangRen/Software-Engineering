...
public class CustomerServiceImpl implements CustomerService {
    ...
    public void createCustomer(CustomerPO customerPO){
        customerPO.setId(generateId(customerPO));
        customerDao.createOne(customerPO);

    }

    @Override
    public void deleteCustomer(Integer supplier){customerDao.deleteOne(supplier);}

    private Integer generateId(CustomerPO customerPO){

        List<CustomerPO> L1 = getCustomersByType(CustomerType.SELLER);
        List<CustomerPO> L2 = getCustomersByType(CustomerType.SUPPLIER);
        return L1.size()+L2.size()+1;
    }
}
